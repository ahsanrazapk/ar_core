...
import com.falcon.ar_core.Serialization.deserializeMatrix4
import com.falcon.ar_core.Serialization.serializeAnchor
import com.falcon.ar_core.Serialization.serializeHitResult
import com.falcon.ar_core.Serialization.serializePose
                        ...
                        anchorName?.let {
                            ...
                            if (ttl != null) {
                                cloudAnchorHandler.hostCloudAnchorWithTtl(
                                    anchorName,
                                    anchorNode!!.anchor,
                                    CustomCloudAnchorUploadedListener(),
                                    ttl
                                )
                            } else {
                    ...
                    "downloadAnchor" -> {
                        ...
                        anchorId?.let {
                            cloudAnchorHandler.resolveCloudAnchor(
                                anchorId,
                                CustomCloudAnchorDownloadedListener()
                            )
                        }
                    }
...
//                        onDestroy()
    ...
    private fun addNode(
        ...
        dictAnchor: HashMap<String, Any>? = null
    ): CompletableFuture<Boolean> {
        val completableFutureSuccess: CompletableFuture<Boolean> = CompletableFuture()

        try {
            when (dictNode["type"] as Int) {
                0 -> { // GLTF2 Model from Flutter asset folder
                    // Get path to given Flutter asset
                    val loader: FlutterLoader = FlutterInjector.instance().flutterLoader()
                    val key: String = loader.getLookupKeyForAsset(dictNode["uri"] as String)
                    ...
                    modelBuilder.makeNodeFromGltf(
                        ...
                        enablePans,
                        enableRotation,
                        dictNode["name"] as String,
                        key,
                        dictNode["transformation"] as ArrayList<Double>
                    )
                        .thenAccept { node ->
                        ...
                        .exceptionally { throwable ->
                            ...
                            val runnable = Runnable {
                                sessionManagerChannel.invokeMethod(
                                    "onError",
                                    listOf("Unable to load renderable" + dictNode["uri"] as String)
                                )
                            }
                            mainHandler.post(runnable)
                ...
                2 -> { // fileSystemAppFolderGLB
                    ...
                    val assetPath = documentsPath + "/app_flutter/" + dictNode["uri"] as String
    ...
    private inner class CustomCloudAnchorUploadedListener : CloudAnchorHandler.CloudAnchorListener {
    ...
    private inner class CustomCloudAnchorDownloadedListener : CloudAnchorHandler.CloudAnchorListener {
        override fun onCloudTaskComplete(anchorName: String?, anchor: Anchor?) {
            val cloudState = anchor!!.cloudAnchorState
            if (cloudState.isError) {
